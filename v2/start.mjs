...
bridge.on('error',e=>{console.error('Local bridge failed:',e.message);server.close();process.exitCode=1;});
bridge.on('exit',code=>{if(code){console.error('Local bridge exited:',code);server.close();process.exitCode=code;} });
function stop(){server.close();bridge.kill();}process.on('SIGINT',stop);process.on('SIGTERM',stop);
