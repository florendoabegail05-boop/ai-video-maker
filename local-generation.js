...
(function () {
  ...
  function assetPath(value) { const a = value?.asset; return a?.path || a?.file || value?.path || value?.file || null; }
  function promptFor(scene, p, kind, previous) {
    ...
    return { prompt: scene.videoPrompt, project: p.name, shot: scene.number, duration, aspectRatio: "9:16", mode: "image-to-video", imageInput: assetPath(previous), continuityHandoff: `Preserve the exact visual state from the source image. ${scene.videoPrompt}` };
  }
  ...
  function assetPathFromVideo(video) { return assetPath(video); }
  async function assembleAll() {
    const p = activeProject(), x = pipelineState(p), clips = x.shots.map(s => assetPathFromVideo(s.generation?.video)).filter(Boolean);
  ...
  function render() {
    ...
    $("lgHealth").onclick = async () => { const status = $("lgStatus"); try { const h = await health(); status.textContent = `✓ Bridge online. FFmpeg: ${h.ffmpeg}. Image: ${h.runners.image?'ready':'not configured'} · Video: ${h.runners.video?'ready':'not configured'} · Voice: ${h.runners.voice?'ready':'not configured'} · Audio: ${h.runners.audio?'ready':'not configured'} · Motion fallback: ${h.motionFallback?.enabled?'ready':'off'}`; } catch (e) { status.textContent = `✕ Bridge unavailable: ${e.message}. Start local-bridge/server.mjs first.`; } };
