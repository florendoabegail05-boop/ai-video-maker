...
function assertLocalUrl(value) {
  ...
  if (parsed.protocol !== 'http:' || !['127.0.0.1', 'localhost'].includes(parsed.hostname)) {
    throw new Error('ComfyUI URL must point to localhost or 127.0.0.1.');
  }
  return parsed;
}
...
function outputRecords(history) {
  ...
  for (const node of Object.values(history?.outputs || {})) {
    for (const type of ['images', 'gifs', 'videos', 'audio']) {
      for (const item of node?.[type] || []) records.push({ ...item, mediaType: type });
    }
  }
  return records;
}
...
async function downloadOutput(base, item, mediaRoot, jobId) {
  ...
  const target = path.join(mediaRoot, `${jobId}-${safe}`);
...
export async function runComfyWorkflow({ base, workflowFile, request, kind, mediaRoot, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  let inputImage = '';
  if (source) inputImage = await uploadImage(baseUrl(comfy), source);

  const vars = {
    PROMPT: clean(request.prompt || request.text),
    INPUT_IMAGE: inputImage,
    WIDTH: String(request.width || (request.aspectRatio === '9:16' ? 576 : 1024)),
    HEIGHT: String(request.height || (request.aspectRatio === '9:16' ? 1024 : 576)),
    FRAMES: String(request.frames || Math.max(16, Math.round((Number(request.duration) || 5) * 16))),
    FPS: String(request.fps || 16),
    SEED: String(request.seed ?? Math.floor(Math.random() * 2147483647)),
    KIND: kind,
    REQUEST_ID: clean(request.requestId || randomUUID())
  };
  ...
  while (Date.now() < deadline) {
    ...
    if (entry.status?.status_str === 'error' || entry.status?.completed === false && entry.status?.status_str === 'error') {
      throw new Error(`ComfyUI workflow failed for ${promptId}.`);
    }
    const outputs = outputRecords(entry);
