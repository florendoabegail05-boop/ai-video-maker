...
export async function assemble(clips, options = {}) {
  ...
  try {
    const width = Math.max(144, Math.min(3840, Number(options.width || 1080))), height = Math.max(144, Math.min(3840, Number(options.height || 1920))), fps = Math.max(1, Math.min(60, Number(options.fps || 30)));
    const normalized = [];
    ...
    if (audio.length) {
      ...
      const padded = audio.map((_, i) => `[${i + 1}:a]apad[a${i}]`).join(';');
      const labels = audio.map((_, i) => `[a${i}]`).join('');
      const filter = `${padded};${labels}amix=inputs=${audio.length}:duration=longest:dropout_transition=2,aresample=48000,loudnorm=I=-16:TP=-1.5:LRA=11[a]`;
    ...
    if (Array.isArray(options.captions) && options.captions.length) {
      ...
      const srtFile = path.join(jobDir, 'captions.srt'); await fs.writeFile(srtFile, srt, 'utf8');
