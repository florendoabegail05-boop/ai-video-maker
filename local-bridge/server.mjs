...
function readMedia(req) { return new Promise((resolve,reject)=>{let size=0;const chunks=[];req.on('data',chunk=>{size+=chunk.length;if(size>20*1024*1024){reject(Object.assign(new Error('Media upload exceeds 20 MB.'),{status:413}));req.destroy();return;}chunks.push(chunk);});req.on('end',()=>resolve(Buffer.concat(chunks)));req.on('error',reject);}); }
...
const server = http.createServer(async (req, res) => {
  ...
  if (importMatch) {
    ...
    let file;
    try {
      const bytes = await readMedia(req);
      if (bytes.length < 128) throw Object.assign(new Error('Media file is empty or too short.'),{status:422});
      const dir = path.join(path.resolve(MEDIA_ROOT),'imports',kind);
      await fsp.mkdir(dir,{recursive:true});file=path.join(dir,randomUUID()+ext);
