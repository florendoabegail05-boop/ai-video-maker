...
async function forward(kind, request) {
  ...
  if (!runner) return { status: 503, body: { status: 'unavailable', kind, error: `No safe local ${kind} runner configured.`, hint: `Set AIVM_${kind.toUpperCase()}_RUNNER to a localhost HTTP endpoint, or configure AIVM_COMFYUI_URL + AIVM_COMFYUI_WORKFLOW_${kind.toUpperCase()}.` } };
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/health') return json(res, 200, { ok: true, service: 'aivm-local-bridge', version: 3, loopbackOnly: true, mock: process.env.AIVM_MOCK === '1', ffmpeg: process.env.AIVM_FFMPEG || 'ffmpeg', mediaRoot: MEDIA_ROOT, runners: Object.fromEntries(Object.entries(RUNNERS).map(([k]) => [k, !!runnerFor(k) || !!comfyFor(k)])), comfyui: { enabled: !!process.env.AIVM_COMFYUI_URL, image: comfyConfigured('image'), video: comfyConfigured('video'), voice: comfyConfigured('voice'), audio: comfyConfigured('audio') } });
