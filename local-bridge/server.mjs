...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url?.startsWith('/v1/import/audio?')) {
    if (!localOrigin(req)) return json(res, 403, { status:'forbidden', error:'Audio import requires a local browser origin.' });
    const name = new URL(req.url, 'http://localhost').searchParams.get('name') || '';
    const ext = path.extname(name).toLowerCase();
    if (!['.wav','.mp3','.m4a','.ogg','.flac'].includes(ext)) return json(res, 415, { status:'invalid', error:'Unsupported audio extension.' });
    let file;
    try { const bytes = await readAudio(req); if(bytes.length<128)throw Object.assign(new Error('Audio file is empty or too short.'),{status:422});const dir=path.join(path.resolve(MEDIA_ROOT),'imports','audio');await fsp.mkdir(dir,{recursive:true});file=path.join(dir,randomUUID()+ext);await fsp.writeFile(file,bytes,{flag:'wx'});const info=await inspectMedia(file);if(!info.audio||info.video)throw Object.assign(new Error('File must contain audio only.'),{status:422});return json(res,200,{status:'completed',path:file,filename:path.basename(file),bytes:info.bytes,duration:info.duration,audio:info.audio}); }
    catch(error){if(file)await fsp.rm(file,{force:true}).catch(()=>{});return json(res,error.status||422,{status:'failed',error:error.message||'Audio import failed.'});}
  }
