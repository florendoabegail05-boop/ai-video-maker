 ...
 * Loopback only; generation uses explicit localhost runners and assembly uses
 * a fixed FFmpeg executable over files inside AIVM_MEDIA_ROOT.
 */
...
function json(res, status, payload) {
  ...
  res.writeHead(status, { 'content-type': 'application/json; charset=utf-8', 'content-length': Buffer.byteLength(body), 'cache-control': 'no-store', 'access-control-allow-origin': 'null', 'access-control-allow-methods': 'GET,POST,OPTIONS', 'access-control-allow-headers': 'content-type' });
  res.end(body);
}
function readJson(req) {
  return new Promise((resolve, reject) => {
    let size = 0; const chunks = [];
    req.on('data', chunk => { size += chunk.length; if (size > MAX_BODY) { reject(Object.assign(new Error('Request body too large.'), { status: 413 })); req.destroy(); return; } chunks.push(chunk); });
    req.on('end', () => { try { resolve(JSON.parse(Buffer.concat(chunks).toString('utf8') || '{}')); } catch { reject(Object.assign(new Error('Request body must be valid JSON.'), { status: 400 })); } });
    req.on('error', reject);
  });
}
function runnerFor(kind) {
  const url = RUNNERS[kind]; if (!url) return null;
  try { const parsed = new URL(url); if (parsed.protocol !== 'http:' || !['127.0.0.1', 'localhost'].includes(parsed.hostname)) return null; return parsed; } catch { return null; }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (exportMatch) {
    const file = exportsByJob.get(exportMatch[1]);
    if (!file) return json(res, 404, { status: 'not_found', error: 'Export job not found or bridge was restarted.' });
    return fs.stat(file, (error, stat) => {
      if (error) return json(res, 404, { status: 'not_found', error: 'Export file is no longer available.' });
      res.writeHead(200, { 'content-type': 'video/mp4', 'content-length': stat.size, 'cache-control': 'no-store', 'access-control-allow-origin': 'null', 'content-disposition': 'attachment; filename="ai-video-maker.mp4"' });
  ...
  if (req.method === 'POST' && req.url === '/v1/assemble') {
    try {
      ...
      exportsByJob.set(result.jobId, result.outputPath);
      while (exportsByJob.size > 10) exportsByJob.delete(exportsByJob.keys().next().value);
      return json(res, 200, { ...result, downloadUrl: `/v1/exports/${result.jobId}` });
    } catch (error) { return json(res, error.status || 502, { status: 'failed', error: error.message || 'Assembly failed.' }); }
  }
  ...
  if (match) {
    ...
    try {
      ...
      const result = await forward(kind, request); return json(res, result.status, result.body);
