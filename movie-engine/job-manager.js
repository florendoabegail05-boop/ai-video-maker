...
function memoryStore() { const data = new Map(); return { get: key => data.get(key) || null, set: (key, value) => data.set(key, value) }; }
function createJobStore(storage) {
  const backend = storage || (typeof localStorage !== "undefined" ? localStorage : memoryStore());
  const read = key => { try { const value = backend.getItem ? backend.getItem(key) : backend.get(key); return typeof value === "string" ? JSON.parse(value) : value; } catch { return null; } };
...
function createJob({ id, projectId, shots = [], maxRetries = 2, metadata = {} } = {}, storage) {
  const store = createJobStore(storage); const jobId = clean(id) || `render-${Date.now().toString(36)}`; const existing = store.read(DEFAULT_KEY) || {};
  if (existing[jobId]) return existing[jobId];
  const now = new Date().toISOString(); const job = { schemaVersion: 2, jobId, projectId: clean(projectId), state: "queued", createdAt: now, updatedAt: now, maxRetries: Math.max(0, Math.min(5, Number(maxRetries) || 2)), metadata, shots: shots.map((shot, i) => ({ shotId: clean(shot.shotId) || `shot-${i + 1}`, state: "queued", attempts: 0, assetPath: null, error: null })) };
  existing[jobId] = job; store.write(DEFAULT_KEY, existing); return job;
}
function updateJob(jobId, patch, storage) {
  const store = createJobStore(storage); const all = store.read(DEFAULT_KEY) || {}; const job = all[clean(jobId)]; if (!job) throw new Error("Production job not found");
...
function updateShot(jobId, shotId, patch, storage) {
  const store = createJobStore(storage); const all = store.read(DEFAULT_KEY) || {}; const job = all[clean(jobId)]; if (!job) throw new Error("Production job not found");
  const index = job.shots.findIndex(s => s.shotId === clean(shotId)); if (index < 0) throw new Error("Shot not found");
  const shot = { ...job.shots[index], ...patch, attempts: Math.max(job.shots[index].attempts, Number(patch?.attempts || 0)) }; job.shots[index] = shot; job.updatedAt = new Date().toISOString(); all[job.jobId] = job; store.write(DEFAULT_KEY, all); return shot;
}
